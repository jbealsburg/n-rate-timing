...
We need to figure out if yield data makes sense (it is very low) and what datasets we have to make comparisons. The delay of applying fall N rates on first year stands complicates analysis.

# Questions

Were the fall treatments applied right after planting or the following year? I assume yes.

Are there plots that received nitrogen in the fall prior to their first harvest? I assume no.

The fall treatments are offset by a year in V17, and complicated by a summer N application in year 1 prior to harvest. Fall treatments may need to be analyzed separately as a treatment only measured on year 2 and 3 kernza stands. At Staples and R100, fall plots received summer N in year 1 prior to harvest. At V17, fall plots received 0N prior to harvest.
...
```{r, include=T,warning=F,fig.cap= "Yields at all 3 sites range from 0-150 kg ha across all years of data collection. We expect yields ranging from 0-1000 kg ha. Why are yields so low?"}
theme_set(theme_bw())
